...
class Metronome: NSObject {
    ...
    func prepareToPlay() {
        ...
        sequencer.tracks[2].setMIDIOutput(callbackInst.midiIn)
    ...
    @objc
    func onQuarterVolumeChange(_ value: Int) {
      quarterVolume = value
      sequencer.tracks[0].clear() 
      for i in 0..<numBeats {
        if i == 0 {
    ...
    @objc
    func onEighthVolumeChange(_ value: Int) {
      eighthVolume = value
      sequencer.tracks[1].clear()
      for i in 0..<numBeats {
        var eighthNotePosition = i + 0.5
    ...
    @objc
    func onSixteenthVolumeChange(_ value: Int) {
        sixteenthVolume = value
        sequencer.tracks[2].clear()
        for i in 0..<numBeats {
            for j in 0..<2 {
                var tempLocation = i + 0.25 + (0.5 * j)
